 ...
 *
 * When a handler throws an exception that is not a [[Halt]], the server invokes
 * [[handleDefect]] to produce a response or a deliberate [[Halt]]. This
 * prevents raw exception leakage to clients.
 *
...
trait DefectHandler {
   ...
   *
   * @param request
   *   the HTTP request being processed when the error occurred
   * @param throwable
   *   the unexpected error
   * @return
   *   a [[Response]] to send to the client, or a [[Halt]] to deliberately

...
/**
 * Version-locked connector views: each class hardcodes its protocol case, so
 * the wire version lives in the static type with no runtime mapping to drift.
...
/** Version-locked connector view for TLS `HTTP/2.0`; see [[H2CConnector]]. */
final class H2Connector(
  bind: BindAddress = BindAddress.Tcp(),
  idleTimeout: java.time.Duration = java.time.Duration.ofSeconds(60),
  maxRequestBodySize: Long = Connector.DefaultMaxRequestBodySize,
  requestTimeoutMs: Long = Connector.DefaultRequestTimeoutMs,
...
/** Version-locked connector view for `HTTP/3.0`; see [[H2CConnector]]. */
final class H3Connector(
  bind: BindAddress = BindAddress.Tcp(),
  idleTimeout: java.time.Duration = java.time.Duration.ofSeconds(60),
  maxRequestBodySize: Long = Connector.DefaultMaxRequestBodySize,
  requestTimeoutMs: Long = Connector.DefaultRequestTimeoutMs,
  ...
  http3: Http3Config = Http3Config(),
) extends Connector(
      bind,
      Protocol.H3(tls, quic, http3),
      idleTimeout,
      maxRequestBodySize,
      requestTimeoutMs,
      headerTimeoutMs,

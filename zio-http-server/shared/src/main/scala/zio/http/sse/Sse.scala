 ...
 * in this repo, next to the server that serves them — as plain factories plus
 * implicit companion syntax (`Body.sse(...)`, `Response.sse(...)`), which
 * resolves through the implicit scope of the imported [[Sse]] object.
 */
object Sse {
   ...
   * [[SseCodec.encode]] as the stream flows (one `flatMap` step per event), so
   * an unbounded event stream never lands on the heap at once. Downstream (Todo
   * 6 `sendStreamedBody`) chunks it by `maxFrameSize` with no Content-Length.
   */
  ...
  /**
   * Builds a `200 OK` event-stream response (`Content-Type: text/event-stream`,
   * `Cache-Control: no-cache`) over a streamed body.
   */

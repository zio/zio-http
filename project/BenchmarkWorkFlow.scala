      ...
      steps = List(
        ...
        WorkflowStep.Run(
          ...
          commands = List(
            ...
            """repoAndSHA=$(echo ${{github.event.pull_request.head.repo.full_name}}.git#${{github.event.pull_request.head.sha}})""",
            "echo ${{github.event.pull_request.head.repo.owner.login}}/zio-http.git#${{github.event.pull_request.head.sha}}",

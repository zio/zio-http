      ...
      steps = List(
        ...
        WorkflowStep.Run(
          ...
          commands = List(
            ...
            """RESULT_CONCURRENCY=$(echo $(grep -B 1 -A 17 "Concurrency: 256 for plaintext" result) | grep -oiE "concurrency: [0-9]+") || true""",
            """echo ::set-output name=request_result::$(echo $RESULT_REQUEST) || true""",
            """echo ::set-output name=concurrency_result::$(echo $RESULT_CONCURRENCY) || true""",
          ),
        ),
        WorkflowStep.Use(
          ref = UseRef.Public("peter-evans", "commit-comment", "v1"),
          cond = Some("${{ github.event.pull_request.head.repo.full_name == 'dream11/zio-http'}}"),
          params = Map(

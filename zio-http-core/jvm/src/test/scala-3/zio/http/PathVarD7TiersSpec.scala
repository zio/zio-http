 ...
 *   - the unused-PathVar WARNING CONTENT, per-var separateness, or the real `-Werror` build-level
 *     proofs (see `PathVarHandlerBindingSpec.scala`, tests 14-16, and its `FatalWarningsProof`
 *     helper object - Todo 6's deliverable), or
...
object PathVarD7TiersSpec extends ZIOSpecDefault {
  ...
  /** Tier-2-only capability type: never a `SegmentCodec`-capturable primitive (Int/Long/String/
    * Boolean/UUID), so it can NEVER be classified as a PathVar candidate - always Context-resolved.
    * Named distinctly from `PathVarHandlerBindingSpec`'s own nested `BasketId` to avoid any
    * cross-file naming confusion, even though the two are not visible to each other (different
    * enclosing objects).
    */
  ...
  def spec = suite("PathVarD7Tiers (Scala 3)")(
    ...
    suite("D7 tier 3 ONLY: Request and Scope, separately and together - zero PathVars in the pattern")(
      ...
      test("Scope alone, zero PathVars, zero Context") {
        val route  = Method.GET / "warmup" -> handler((scope: Scope) => Response.text(if (scope != null) "scope-received" else "no-scope"))
      ...
      test("Request AND Scope together, zero PathVars, zero Context") {
        val route  = Method.GET / "combined" ->
          handler((request: Request, scope: Scope) => Response.text(s"${request.method}+${if (scope != null) "scope" else "none"}"))

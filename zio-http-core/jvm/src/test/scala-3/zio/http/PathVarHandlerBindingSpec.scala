 ...
 * `.omo/drafts/route-pattern-typed-vars.md`'s "Worked examples" section, in the
 * FINAL `pattern -> handler(fn)` syntax (D9/D12), on Scala 3.
 */
  ...
  def spec = suite("PathVarHandlerBinding (Scala 3)")(
    ...
    test("8. PathVar name+type miss falls through to Context[Ctx] by type") {
      val route      = Method.GET / uuid("customerId") ->
        handler((customerId: UUID, basketId: BasketId) =>
          Response.text(s"customer=$customerId basket=${basketId.value}"),
        )
...
"""
    ...
    test(
      "12. multiple unused vars in a 3-segment pattern still run correctly (separateness of the warnings is verified below via a real -Werror compile)",
    ) {
    ...
    test(
      "15 (-Werror build-level proof). the SAME pattern with full use compiles cleanly (zero warnings) under fatal-warnings",
    ) {
      val code               =
 ...
 * informational side effect - since a genuine external `dotc` process, given
 * the real production `RouteBinding.scala`, either fails or succeeds to compile
 * based on it.
 */
private object FatalWarningsProof {
    ...
    def search(d: java.io.File): List[java.io.File] =
      Option(d.listFiles()).toList.flatten.flatMap { f =>
        if (f.isDirectory) search(f)
          ...
          f.getName.startsWith(artifactPrefix) && f.getName
            .endsWith(".jar") && !f.getName.contains("sources") && !f.getName.contains("javadoc")
        )
          List(f)
        else Nil
      }
  ...
  def compileScala3(source: String): (Int, String) = {
    ...
    val runtimeClasspath = sys.props("java.class.path").split(java.io.File.pathSeparator).toSeq.filterNot { entry =>
      val normalized = entry.replace('\\', '/')
      normalized.contains("/org/scala-lang/scala3-library_") ||
      normalized.contains("/org/scala-lang/scala-library/") ||
      normalized.contains("/org/scala-lang/scala3-interfaces/") ||
      normalized.contains("/org/scala-lang/tasty-core_") ||

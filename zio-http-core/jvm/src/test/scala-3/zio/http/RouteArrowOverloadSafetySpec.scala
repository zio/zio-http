 ...
 * (D9/D12) and D12's "no Route-level middleware" boundary on Scala 3.
 *
 * Scala 3's `->` is a SINGLE `transparent inline` macro (not two literal overloads) that
 * dispatches internally on the TYPE of its argument (see `RouteBinding.scala`'s module doc) - so
 * there is no possibility of the compiler reporting an "ambiguous overload" error between the
 * macro-derived-`handler(fn)` path and the pre-built-`Handler[Ctx,V]` path; both shapes are
 * accepted by the SAME single extension method. This spec proves both shapes compile and run
 * correctly side-by-side (including combined in one `Routes(...)`), and that the D12 safety net
 * (bare `pattern -> handler(fn) @@ mw`, without a `Routes(...)` wrapper, must NOT silently compile)
 * really does fail immediately, with the exact compiler error captured and asserted on.
 */
  ...
  def spec = suite("RouteArrowOverloadSafety (Scala 3)")(
    suite("(a) the two `->` call shapes never collide ambiguously")(
      ...
      test("both shapes combine in a single Routes(...) with zero ambiguity/interference") {
        ...
        assertTrue(routes.size == 2, macroResult == Response.text("macro:9"), prebuiltResult == Response.text("prebuilt-pong"))
      },
      test("a pre-built Handler whose Vars type is a real (non-phantom) tuple is never misidentified as a macro-derived handler") {
...
"""
      ...
      test("bare `pattern -> Handler.succeed(...) @@ mw` (pre-built shape) fails the same way - `@@` binds to the Handler") {
        assertZIO(typeCheck {
          """import zio.blocks.endpoint.PathCodec._
import zio.blocks.endpoint.RoutePattern.{MethodSyntax, RoutePatternOps}

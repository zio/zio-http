 ...
 * boundary on Scala 2.13.
 *
 * Unlike Scala 3 (a single `transparent inline` macro dispatching internally),
 * Scala 2.13's `->` genuinely IS two declared overloads on
 * `RoutePatternArrowOps` (`RouteBinding.scala`):
 *   - `def ->[Ctx, Req](h: Handler[Ctx, Req]): Route[Ctx] = macro ...`
 *     (macro-derived path)
 *   - `def ->[Ctx](h: Handler[Ctx, A]): Route[Ctx]` (pre-built-Handler
 *     passthrough, no macro)
 * This spec proves ordinary Scala overload resolution picks the correct one for
 * each call shape with ZERO "ambiguous reference to overloaded definition"
 * errors, that both shapes run correctly (including combined in one
 * `Routes(...)`), and that the D12 safety net (bare
 * `pattern -> handler(fn) @@ mw` without a `Routes(...)` wrapper) really does
 * fail immediately, with the exact compiler error captured and asserted on -
 * identical in shape to Scala 3's behavior (see the Scala 3 sibling spec's
 * module doc for the operator-precedence finding).
 */
object RouteArrowOverloadSafetySpec extends ZIOSpecDefault {
  ...
  def spec: Spec[Any, Nothing] = suite("RouteArrowOverloadSafety (Scala 2.13)")(
    suite("(a) the two `->` overloads never collide ambiguously")(
      ...
      test(
        "a pre-built Handler over a real 2-segment value tuple resolves to the non-macro overload, not the macro one",
      ) {
...
"""
      ...
      test(
        "bare `pattern -> Handler.succeed(...) @@ mw` (pre-built overload) fails the same way - `@@` binds to the Handler",
      ) {
        assertZIO(typeCheck {
          """import zio.blocks.endpoint.PathCodec._
import zio.blocks.endpoint.RoutePattern.MethodSyntax

...
trait QueryParams {
   ...
   */
  def encode(encoding: QueryParamEncoding): String = encode(Charsets.Utf8, encoding)

  /**
   * Encodes the query parameters into a string using the specified charset.
   */
  def encode(charset: Charset, encoding: QueryParamEncoding): String =
    encoding.encode("", self, charset)
...
object QueryParams {
   ...
   */
  def decode(queryStringFragment: String, encoding: QueryParamEncoding, charset: Charset = Charsets.Utf8): QueryParams =
    encoding.decode(queryStringFragment, charset)

...
sealed trait Cookie { self =>
   ...
   */
  final def encode(implicit encoding: CookieEncoding): Either[Exception, String] =
    encodeValidate(validate = false)
   ...
   * specified.
   */
  def encodeValidate(validate: Boolean)(implicit encoding: CookieEncoding): Either[Exception, String]
...
object Cookie {
  ...
  def decodeRequest(header: String, validate: Boolean = false)(implicit encoding: CookieEncoding): Either[Exception, Chunk[Cookie.Request]] =
    Cookie.Request.decode(header, validate)

  def decodeResponse(header: String, validate: Boolean = false)(implicit encoding: CookieEncoding): Either[Exception, Cookie.Response] =
  ...
  final case class Response(
    ...
    override def encodeValidate(validate: Boolean)(implicit encoding: CookieEncoding): Either[Exception, String] =
      try {
        Right(encoding.encodeResponseCookie(self, validate))
      } catch {
  ...
  object Response {
     ...
     */
    def decode(header: String, validate: Boolean = false)(implicit encoding: CookieEncoding): Either[Exception, Cookie.Response] = {
      try {
        Right(encoding.decodeResponseCookie(header, validate))
      } catch {
        case e: Exception =>
          Left(e)
      }
  ...
  final case class Request(name: String, content: String) extends Cookie { self =>
    override def encodeValidate(validate: Boolean)(implicit encoding: CookieEncoding): Either[Exception, String] =
      try {
        Right(encoding.encodeRequestCookie(self, validate))
      } catch {
        case e: Exception => Left(e)
      }
  ...
  object Request {
     ...
     */
    def decode(header: String, validate: Boolean = false)(implicit encoding: CookieEncoding): Either[Exception, Chunk[Cookie.Request]] = {
      try {
        Right(encoding.decodeRequestCookie(header, validate))
      } catch {
        case e: Exception =>
          Left(e)
      }

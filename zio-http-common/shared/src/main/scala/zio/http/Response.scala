...
import zio.http.internal.{BodyEncoding, HeaderOps}
...
object Response {
   ...
   */
  def html(data: Html, status: Status = Status.Ok)(implicit bodyEncoding: BodyEncoding): Response =
    Response(
      status,
      contentTypeHtml,
   ...
   */
  def json(data: CharSequence)(implicit bodyEncoding: BodyEncoding): Response =
    Response(
      Status.Ok,
      contentTypeJson,
   ...
   */
  def text(text: CharSequence)(implicit bodyEncoding: BodyEncoding): Response =
    Response(
      Status.Ok,
      contentTypeText,

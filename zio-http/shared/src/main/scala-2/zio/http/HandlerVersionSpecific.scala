...
trait HandlerVersionSpecific {
  private[http] final class ApplyContextAspect[-Env, +Err, -In, +Out, Env0](private val self: Handler[Env, Err, In, Out]) {
    def apply[Env1, Ctx: Tag, In1 <: In](aspect: HandlerAspect[Env1, Ctx])(implicit
      ...
      if (aspect eq HandlerAspect.identity) self.asInstanceOf[Handler[Env0 with Env1, Response, Request, Response]]
      else

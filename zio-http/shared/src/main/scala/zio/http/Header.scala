...
object Header {
  ...
  object ContentSecurityPolicy extends HeaderType {
    ...
    def apply(directive: Directive, directives: Directive*): ContentSecurityPolicy = ContentSecurityPolicy(directive :: directives.toList)

    private def sequence[A, B](s: Array[Either[A, B]]): Either[A, List[B]] =
    ...
    def parse(value: String): Either[String, ContentSecurityPolicy] = {
      val directives = value.split(';').map(directive => Directive.parse(directive.trim))
      sequence(directives).map(ContentSecurityPolicy(_))
    }

    def render(csp: ContentSecurityPolicy): String =
      csp.directives.map(Directive.render).mkString("; ")
    ...
    object Directive {
      final case class SourcePolicy(srcType: SourcePolicyType, src: Source) extends Directive

      case object BlockAllMixedContent extends Directive

      // TODO: Deprecated and only Safari supports this and it is non-standard. Should we remove it?
      final case class PluginTypes(value: String) extends Directive

      // TODO: no modern browser supports this. Should we remove it?
      final case class Referrer(referrer: ReferrerPolicy) extends Directive

      final case class ReportTo(groupName: String) extends Directive

      final case class ReportUri(uri: URI) extends Directive

      final case class RequireSriFor(requirement: RequireSriForValue) extends Directive

      final case class Sandbox(value: SandboxValue) extends Directive

      final case class TrustedTypes(value: TrustedTypesValue) extends Directive

      case object UpgradeInsecureRequests extends Directive
      ...
      def parse(value: String): Either[String, Directive] =
        value match {
          case "block-all-mixed-content"       => Right(Directive.BlockAllMixedContent)
          case PluginTypesRegex(types)         => Right(Directive.PluginTypes(types))
          case ReferrerRegex(referrer)         => ReferrerPolicy.parse(referrer).map(Directive.Referrer(_)).toRight("Invalid referrer policy")
          case ReportToRegex(group)            => Right(Directive.ReportTo(group))
          case ReportUriRegex(uri)             => Try(new URI(uri)).map(Directive.ReportUri(_)).toEither.left.map(_ => "Invalid report-uri")
          case RequireSriRegex(value)          => RequireSriForValue.parse(value).map(Directive.RequireSriFor(_)).toRight("Invalid require-sri-for value")
          case TrustedTypesRegex(value)        => TrustedTypesValue.parse(value).map(Directive.TrustedTypes(_)).toRight("Invalid trusted-types value")
          case SandboxRegex(sandbox)           => SandboxValue.parse(sandbox).map(Directive.Sandbox(_)).toRight("Invalid sandbox value")
          case "upgrade-insecure-requests"     => Right(Directive.UpgradeInsecureRequests)
          case PolicyRegex(policyType, policy) => Directive.fromTypeAndPolicy(policyType, policy)
          case _                               => Left("Invalid Content-Security-Policy directive")

        }

      def render(csp: Directive): String =
        csp match {
          case Directive.BlockAllMixedContent    => "block-all-mixed-content"
          case Directive.PluginTypes(types)      => s"plugin-types $types"
          case Directive.Referrer(referrer)      => s"referrer ${ReferrerPolicy.render(referrer)}"
          case Directive.ReportTo(reportTo)      => s"report-to $reportTo"
          case Directive.ReportUri(uri)          => s"report-uri $uri"
          case Directive.RequireSriFor(value)    => s"require-sri-for ${RequireSriForValue.fromRequireSriForValue(value)}"
          case Directive.TrustedTypes(value)     => s"trusted-types ${TrustedTypesValue.fromTrustedTypesValue(value)}"
          case Directive.Sandbox(value)          => s"sandbox ${SandboxValue.render(value)}"
          case Directive.UpgradeInsecureRequests => "upgrade-insecure-requests"
          case SourcePolicy(policyType, policy)  => s"${SourcePolicyType.render(policyType)} ${Source.render(policy)}"
        }

      def fromTypeAndPolicy(policyType: String, policy: String): Either[String, Directive] =
        SourcePolicyType
          .parse(policyType)

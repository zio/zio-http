      ...
      val combiner: ProtocolStack[Env1, (Request, CtxOut), (Request, zippable.Out), Response, Response] =
        ProtocolStack.interceptHandlerStateful[
          ...
          Handler
            ...
            .asInstanceOf[Handler[Env1, Response, (Request, CtxOut), (that.protocol.State, (Request, zippable.Out))]],
        )(
          Handler
            .fromFunctionZIO[(that.protocol.State, Response)] { case (state, either) =>
              that.protocol.outgoing(state, either)
            }
            .asInstanceOf[Handler[Env1, Nothing, (that.protocol.State, Response), Response]],
        )

...
private[http] final case class ErrorConstructor(
  missing: String => HttpCodecError,
  missingAll: Chunk[String] => HttpCodecError,
  invalid: Chunk[ValidationError] => HttpCodecError,
  malformed: (String, DecodeError) => HttpCodecError,
  invalidCount: (String, Int, Int) => HttpCodecError,
)

private[http] final case class StringSchemaCodec[A, Target](
  private[http] val schema: Schema[A],
  private[http] val add: (Target, String, String) => Target,
  private[http] val addAll: (Target, Iterable[(String, String)]) => Target,
  private[http] val contains: (Target, String) => Boolean,
  private[http] val unsafeGet: (Target, String) => String,
  private[http] val getAll: (Target, String) => Chunk[String],
  private[http] val count: (Target, String) => Int,
  private[http] val error: ErrorConstructor,
  private[http] val kebabCase: Boolean,
  private[http] val defaultValue: A,
  private[http] val isOptional: Boolean = false,
  private[http] val isOptionalSchema: Boolean = false,
) {
        ...
        case (field, codec)                                                       =>
          ...
          val decoded = {
            if (value == null || (value == "" && !emptyStringIsValue(codec.schema) && codec.isOptional))
              codec.defaultValue
            else decodeAndUnwrap(field, codec, value, error.malformed.apply)
          }
...
object StringSchemaCodec {
  ...
  private[http] final case class PrimitiveCodec[A](
    ...
    private[http] def optional = copy(schema.optional)

    private[http] val decode: String => A =
      PrimitiveCodec.primitiveSchemaDecoder(schema)

    private[http] def validate(value: A) =
  ...
  @tailrec
  private[http] def fromSchema[A, Target](
    schema: Schema[A],
    add: (Target, String, String) => Target,
    addAll: (Target, Iterable[(String, String)]) => Target,
    contains: (Target, String) => Boolean,
    unsafeGet: (Target, String) => String,
    unsafeGetAll: (Target, String) => Chunk[String],
    count: (Target, String) => Int,
    error: ErrorConstructor,
    isKebabCase: Boolean,
    name: String,
  ): StringSchemaCodec[A, Target] = {
    val defaultValue     = StringSchemaCodec.defaultValue(schema)
    val isOptional       = StringSchemaCodec.isOptional(schema)
    val isOptionalSchema = StringSchemaCodec.isOptionalSchema(schema)

    def stringSchemaCodec(schema: Schema[Any]): StringSchemaCodec[A, Target] =
      StringSchemaCodec(

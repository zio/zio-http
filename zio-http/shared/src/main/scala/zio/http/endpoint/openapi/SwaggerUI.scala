...
object SwaggerUI {
   ...
   */
  //format: on
  @deprecated("Use app instead. Will be removed with the next release.")
   ...
   */
  // format: on
  def app(path: PathCodec[Unit], api: OpenAPI, apis: OpenAPI*): HttpApp[Any, Response] = {
    app(path, DefaultSwaggerUIVersion, api, apis: _*)
  }
   ...
   */
  // format: on
  def app(path: PathCodec[Unit], version: String, api: OpenAPI, apis: OpenAPI*): HttpApp[Any, Response] = {
    import zio.http.template._
    val basePath   = Method.GET / path
    val jsonRoutes = (api +: apis).map { api =>
      basePath / s"${URLEncoder.encode(api.info.title, Charsets.Utf8.name())}.json" -> handler { (_: Request) =>
    ...
    HttpApp.fromIterable(jsonRoutes) :+ uiRoute
  }
}

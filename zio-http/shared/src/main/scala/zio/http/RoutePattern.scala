...
object RoutePattern                                                       {
  ...
  private[http] final case class Tree[Env](
    ...
    /**
     * Auto-generates HEAD routes from GET routes. If a HEAD route already
     * exists for a path, it takes precedence over the auto-generated one.
     */
    def withAutoGeneratedHeadRoutes: Tree[Env] = {
      if (getRoot == null) {
        // No GET routes, nothing to auto-generate
        self
      } else {
        // Copy GET routes to HEAD, but only if HEAD doesn't already have a handler for that path
        // Note: getRoot ++ headRoot ensures explicit HEAD routes (headRoot) take precedence
        val newHeadRoot = if (headRoot == null) getRoot else getRoot ++ headRoot
        self.copy(headRoot = newHeadRoot)
      }
    }
  }

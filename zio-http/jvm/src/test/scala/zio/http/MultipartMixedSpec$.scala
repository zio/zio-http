...
import zio.test.{Assertion, Gen, Spec, TestAspect, TestEnvironment, check, checkAll}
...
object MultipartMixedSpec$ extends ZIOHttpSpec {
  ...
  val mixedSuite = suiteAll("multipart/mixed") {
    ...
    test("empty test case") {
      val tc = TestCase(Chunk(),
        Boundary("(((b010903d-0d3d-4d22-b5c4-f3f0bf574468)))",Charsets.Utf8),
        Chunk(),
        Chunk(),
        7412,
        gens.breaker.alternating(Chunk(10,10)),
        Chunk(45,45,40,40,40,98,48,49,48,57,48,51,100,45,48,100,51,100,45,52,100,50,50,45,98,53,99,52,45,102,51,102,48,98,102,53,55,52,52,54,56,41,41,41,45,45))
    ...
    test("closing boundary trailing crlf is split between two reads") {
      val tc = TestCase(Chunk(44),
        ...
        Chunk(44,13,10,45,45,40,40,40,98,97,52,53,56,52,102,56,45,99,97,49,54,45,52,53,55,97,45,57,50,49,101,45,97,100,102,49,97,48,49,57,57,54,97,55,41,41,41,13,10,13,10,13,10,45,45,40,40,40,98,97,52,53,56,52,102,56,45,99,97,49,54,45,52,53,55,97,45,57,50,49,101,45,97,100,102,49,97,48,49,57,57,54,97,55,41,41,41,45,45,13,10,-59,119,33,117,-6,15,101)
      )
      tc.testFromRepr
    }
  ...
  case class TestCase(preamble : Chunk[Byte],
                      boundary : Boundary,
    ...
    def this(preamble : Chunk[Byte],
             boundary : Boundary,
             parts : Chunk[TestPart],
             epilogue : Chunk[Byte],
             bufferSize : Int,
             breaker : Breaker) =
      this(preamble, boundary, parts, epilogue, bufferSize, breaker, {
        {
          preamble match {
  ...
  class Breaker(val breakerPl :  ZPipeline[Any, Nothing, Byte, Byte],
  ...
  object gens {
    ...
    object breaker {
      ...
      def alternatingPl(sizes : Chunk[Int]): ZPipeline[Any, Nothing, Byte, Byte] = {
        ...
        def ch(buffered : Chunk[Byte],
               leftovers : Chunk[Byte],
               rem : Int,
               nextSz : ZIO[Any, Nothing, Int]) : ZChannel[Any, ZNothing, Chunk[Byte], Any, Nothing, Chunk[Byte], Any] = {
          if(0 == rem)
            ...
            ZChannel
              .readWithCause(
                in =>
                  ch(buffered, in, rem, nextSz),
                ZChannel.refailCause(_),
                _ =>
                  if(buffered.nonEmpty) ZChannel.write(buffered) else ZChannel.unit
              )
        }
        ...
        ZPipeline
          .unwrapScoped[Any]{
            s0Pull
              ...
              .map{
                case (nextSz, firstSz) =>
                  val channel: ZChannel[Any, ZNothing, Chunk[Byte], Any, Nothing, Chunk[Byte], Any] = ch(Chunk.empty, Chunk.empty, firstSz, nextSz)
                  val pl: ZPipeline[Any, Nothing, Byte, Byte] = channel.toPipeline
                  pl
              }
          }
      }
      def alternating(sizes : Chunk[Int]): Breaker = new Breaker(alternatingPl(sizes), s"alternating(${sizes.mkString("Chunk(", ",", ")")})")

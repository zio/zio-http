  ...
  val routes: Routes[Any, Response] =
    Routes(
      ...
      Method.GET / "sse" / "non-streaming" ->
        handler(Response.fromServerSentEvent(ServerSentEvent(ISO_LOCAL_TIME.format(LocalDateTime.now), retry = Some(10.milliseconds)))),
        Method.GET / "sse" / "collection" ->
        handler(Response.fromServerSentEvents(
          Chunk.fromIterable(

...
import zio.{Task, Trace, UIO, ZIO} // scalafix:ok;

final case class ChannelForUserSocketAppsNetty[-A](
  private val channel: JChannel,
  private val convert: A => Any,
) extends ChannelForUserSocketApps[A] {
  self =>
  ...
  override def contramap[A1](f: A1 => A): ChannelForUserSocketAppsNetty[A1] = copy(convert = convert.compose(f))
...
object ChannelForUserSocketAppsNetty {
  def make[A](channel: JChannel): ChannelForUserSocketAppsNetty[A] = ChannelForUserSocketAppsNetty(channel, identity)
}

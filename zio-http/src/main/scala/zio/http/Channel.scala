...
trait Channel[-In, +Out] { self =>
   ...
   */
  final def map[Out2](f: Out => Out2)(implicit trace: zio.http.Trace): Channel[In, Out2] =
    new Channel[In, Out2] {
      def awaitShutdown: UIO[Unit]              =
        self.awaitShutdown
   ...
   * function.
   */
  final def receiveAll[Env](f: Out => ZIO[Env, Throwable, Any])(implicit
    trace: zio.http.Trace,
  ): ZIO[Env, Throwable, Nothing] =

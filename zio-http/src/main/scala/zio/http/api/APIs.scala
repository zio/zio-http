...
object APIs             {
  def apply(api: API[_, _, _, _]): APIs[api.Id] = Single(api)

  private final case class Single[Id, A, B, C, D](api: API[A, B, C, D])        extends APIs[Id]

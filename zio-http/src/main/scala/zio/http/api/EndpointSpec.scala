 ...
 */
final case class EndpointSpec[Input, Err, Output](
  ...
  def apply(input: Input): Invocation[this.type, Input, Err, Output] =
    Invocation(self, input)

  def apply[A, B](a: A, b: B)(implicit
    ev: (A, B) <:< Input,
  ): Invocation[this.type, Input, Err, Output] =
   ...
   */
  def ??(that: Doc): EndpointSpec[Input, Err, Output] = copy(doc = self.doc + that)
   ...
   */
  def implement[R](f: Input => ZIO[R, Err, Output]): Endpoints[R, Err, this.type] =
    Endpoints.HandledEndpoint[R, Err, Input, Output, this.type](self, f)

  def header[A](codec: HeaderCodec[A])(implicit
    combiner: Combiner[Input, A],
  ): EndpointSpec[combiner.Out, Err, Output] =
   ...
   */
  def out[Output2: Schema]: EndpointSpec[Input, Err, Output2] =
  ...
  ): EndpointSpec[Input, Err, combiner.Out] =
   ...
   * output type.
   */
  def outStream[Output2: Schema]: EndpointSpec[Input, Err, ZStream[Any, Throwable, Output2]] =
...
object EndpointSpec {
   ...
   * definition of the API.
   */
  def get[Input](route: RouteCodec[Input]): EndpointSpec[Input, Unused, Unit] =
    EndpointSpec(route ++ MethodCodec.get, HttpCodec.empty, HttpCodec.unused, Doc.empty)
   ...
   * definition of the API.
   */
  def post[Input](route: RouteCodec[Input]): EndpointSpec[Input, Unused, Unit] =
    EndpointSpec(route ++ MethodCodec.post, HttpCodec.empty, HttpCodec.unused, Doc.empty)
   ...
   * definition of the API.
   */
  def put[Input](route: RouteCodec[Input]): EndpointSpec[Input, Unused, Unit] =
    EndpointSpec(route ++ MethodCodec.put, HttpCodec.empty, HttpCodec.unused, Doc.empty)
}

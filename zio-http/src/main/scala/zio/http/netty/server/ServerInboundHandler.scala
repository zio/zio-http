    ...
    jReq match {
      case jReq: FullHttpRequest =>
        ...
        runtime.runtime(ctx).unsafe.run {
          ...
          webSocketApp.run(webSocketChannel).ensuring(webSocketChannel.shutdown).forkDaemon
        }
        ctx
          .channel()
          .pipeline()
          .addLast(
            new WebSocketServerProtocolHandler(NettySocketProtocol.serverBuilder(SocketProtocol.default).build()),
          )

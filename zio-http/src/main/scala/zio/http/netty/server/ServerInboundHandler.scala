...
trait ServerInboundHandler {
  def create(): SimpleChannelInboundHandler[HttpObject]
}

object ServerInboundHandler {
  @Sharable
  private[zio] final case class ServerInboundHandlerImpl(
    app: HttpApp[Any, Throwable],
    config: ServerConfig,
    errCallback: Option[Server.ErrorCallback],
          ...
          _        <-
            if (response.isWebSocket) ZIO.attempt(upgradeToWebSocket(jRequest, response, runtime))
              ...
              for {
                ...
                flushed   <-
                  if (!jResponse.isInstanceOf[FullHttpResponse]) response.body.write(ctx) else ZIO.succeed(true)
  ...
  val layer: ZLayer[
    ...
    ZLayer.fromZIO {
      ...
      } yield new ServerInboundHandler {
        override def create(): SimpleChannelInboundHandler[HttpObject] =
          ServerInboundHandlerImpl(appRef.get(), config, errCallback.get(), rtm, time)
      }
    }
  }

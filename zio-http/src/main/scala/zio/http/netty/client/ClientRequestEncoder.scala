...
trait ClientRequestEncoder {
  ...
  def encode(req: Request)(implicit trace: Trace): Task[FullHttpRequest] =
    ZIO.attempt(Versions.convertToZIOToNetty(req.version)(Unsafe.unsafe)).flatMap { jVersion =>
      req.body.asChunk.map { chunk =>
        ...
        val headers = req.url.host match {
          case Some(host) =>
            encodedReqHeaders.set(HttpHeaderNames.HOST, req.url.port.fold(host)(port => s"$host:$port"))

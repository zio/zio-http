...
object Content {
  ...
  case object Echo                                                      extends Content[Any, Nothing, Nothing]
  final case class Complete[A](data: A)                                 extends Content[Any, Nothing, A]
  final case class Streaming[R, E, A](stream: ZStream[R, Option[E], A]) extends Content[R, E, A]
  ...
  def fromStream[R, E, A](stream: ZStream[R, E, A]): Content[R, E, A]      = Streaming(stream.mapError(Option(_)))
  def fromSocket[R, E](socketApp: SocketApp[R, E]): Content[R, E, Nothing] = FromSocket(socketApp)
  def echo: Content[Any, Nothing, Nothing]                                 = Echo
}

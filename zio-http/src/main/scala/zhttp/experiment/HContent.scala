...
object HContent {
  ...
  case class FromChannel[R, E, A](ch: HChannel[R, E, A, A])       extends HContent[R, E, A]

  def empty: HContent[Any, Nothing, Nothing]                                  = Empty
  def complete[A](data: A): HContent[Any, Nothing, A]                         = Complete(data)
  def from[R, E, A](ch: HChannel[R, E, A, A]): HContent[R, E, A]              = FromChannel(ch)
  def fromStream[R, E, A](stream: ZStream[R, E, A]): HContent[R, E, A]        = Streaming(stream.mapError(Option(_)))
  def from[R, E, A](cb: (A, Context[A]) => ZIO[R, E, Any]): HContent[R, E, A] = from(HChannel[R, E, A, A](onRead = cb))

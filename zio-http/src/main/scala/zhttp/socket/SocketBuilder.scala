...
sealed trait SocketBuilder[-R, +E] { self =>
  def ++[R1 <: R, E1 >: E](other: SocketBuilder[R1, E1]): SocketBuilder[R1, E1] = SocketBuilder.Concat(self, other)
  def settings: SocketBuilder.Settings[R, E]                                    = SocketBuilder.settings(self)
}
...
object SocketBuilder {
  ...
  private case class SubProtocol(name: String)                                         extends SocketBuilder[Any, Nothing]
  private case class OnOpen[R, E](onOpen: Connection => ZIO[R, E, Unit])               extends SocketBuilder[R, E]
  private case class OnMessage[R, E](onMessage: WebSocketFrame => ZStream[R, E, WebSocketFrame])
      extends SocketBuilder[R, E]
  private case class OnError[R](onError: Throwable => ZIO[R, Nothing, Unit])           extends SocketBuilder[R, Nothing]

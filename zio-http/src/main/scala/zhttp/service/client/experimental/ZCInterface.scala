...
class ZCInterface {
   ...
   *   and kept alive or termainated based on configurations. In case of connection getting terminated attempt to read
...
//    def run[A](req: ZIO[?,?,Request])(f: Response => Task[A]): Task[A] = ???
  ...
  /**
   * TBD Submits a request and decodes the response on success. On failure, the status code is returned. The underlying
   * HTTP connection is closed at the completion of the decoding.
   */

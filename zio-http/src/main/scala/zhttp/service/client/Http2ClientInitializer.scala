...
class Http2ClientInitializer(
  ...
  override def initChannel(ch: SocketChannel): Unit = {
    ...
    val connectionHandler: HttpToHttp2ConnectionHandler = new HttpToHttp2ConnectionHandlerBuilder()
      .frameListener(
    ...
    if (scheme == "https") {
      pipeline.addLast(
        ClientSSLHandler.ssl(sslOption, true).newHandler(ch.alloc()),
        connectionHandler,
        settingsHandler,
        rh,
      )
   ...
   */
  final private class UpgradeRequestHandler(sh: JChannelHandler, rh: JChannelHandler)

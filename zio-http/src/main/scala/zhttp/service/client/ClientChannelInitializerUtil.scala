...
case object ClientChannelInitializerUtil {
  ...
  def configureSsl(ch: Channel,
                           ...
                           enableHttp2: Boolean,
                           jReq: FullHttpRequest): Unit = {
    val pipeline = ch.pipeline
    pipeline.addFirst(SSL_HANDLER,ClientSSLHandler.ssl(sslOption, enableHttp2).newHandler(ch.alloc))
    if (enableHttp2) {
      pipeline.addLast(HTTP2_OR_HTTP_CLIENT_HANDLER,Http2OrHttpClientHandler(ch,connectionHandler,settingsHandler,httpResponseHandler,http2ResponseHandler,jReq))
      ()
    } else {
      ch
        .pipeline()
   ...
   */
  def configureClearText(ch: Channel,
                         settingsHandler: Http2SettingsHandler,
                         connectionHandler: HttpToHttp2ConnectionHandler,
                         httpResponseHandler: ChannelHandler,
                         http2ResponseHandler: Http2ClientResponseHandler,
    ...
    if (enableHttp2) {
      ...
      ch.pipeline.addLast(CLIENT_UPGRADE_REQUEST_HANDLER, new UpgradeRequestHandler(settingsHandler,http2ResponseHandler,jReq))
      ch.pipeline().addLast(CLIENT_CLEAR_TEXT_HTTP2_FALLBACK_HANDLER,ClientClearTextHttp2FallbackHandler(httpResponseHandler))
      ()
    }
    else {
      ch.pipeline
   ...
   */
  final private class UpgradeRequestHandler(settingsHandler: Http2SettingsHandler,
                                            http2ResponseHandler: Http2ClientResponseHandler,

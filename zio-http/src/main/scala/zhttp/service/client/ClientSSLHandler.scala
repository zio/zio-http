...
import io.netty.handler.ssl.{ApplicationProtocolConfig, ApplicationProtocolNames, SupportedCipherSuiteFilter, SslContext => JSslContext, SslContextBuilder => JSslContextBuilder}
...
case object ClientSSLHandler {
  ...
  def ssl(sslOption: ClientSSLOptions,http2:Boolean): JSslContext = {
    ...
    if (http2) builder.ciphers(Http2SecurityUtil.CIPHERS, SupportedCipherSuiteFilter.INSTANCE).applicationProtocolConfig(new ApplicationProtocolConfig(
      Protocol.ALPN,
      ...
      .build();
    else builder.applicationProtocolConfig(new ApplicationProtocolConfig(
      Protocol.ALPN,
      SelectorFailureBehavior.NO_ADVERTISE,
      SelectedListenerFailureBehavior.ACCEPT,
      ApplicationProtocolNames.HTTP_1_1))

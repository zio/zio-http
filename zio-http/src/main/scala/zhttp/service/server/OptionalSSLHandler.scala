  ...
  override def decode(context: ChannelHandlerContext, in: ByteBuf, out: util.List[AnyRef]): Unit = {
    if (in.readableBytes < 5)
      ...
      settings.sslOption.httpBehaviour match {
        ...
        case _                       =>
          context.channel().pipeline().remove(HTTP2_OR_HTTP_SERVER_HANDLER)
          context.channel().pipeline().replace(this, HTTP_SERVER_CODEC, new HttpServerCodec)
          context
            .channel()
            .pipeline()
          ...
          if (settings.acceptContinue)
            context
              .channel()
              .pipeline()
              .addAfter(HTTP_SERVER_CODEC, HTTP_SERVER_EXPECT_CONTINUE, new HttpServerExpectContinueHandler())
          ()
      }
    }
  }

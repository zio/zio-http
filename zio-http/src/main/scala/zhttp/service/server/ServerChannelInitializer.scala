...
import io.netty.handler.codec.http.{HttpServerUpgradeHandler, HttpServerKeepAliveHandler => JHttpServerKeepAliveHandler}
    ...
    if (settings.enableHttp2 == false) {
      ...
      if (sslctx != null) {
        channel
          ...
          .addFirst(
            SSL_HANDLER,
            new OptionalSSLHandler(sslctx.build(), settings.sslOption.httpBehaviour),
          )
        ()
      }
      channel
      ...
      if (sslctx != null) {
        channel.pipeline().addLast(
          sslctx.ciphers(Http2SecurityUtil.CIPHERS, SupportedCipherSuiteFilter.INSTANCE)
      ...
      else
        {
          ...
          p.addLast(new SimpleChannelInboundHandler[HttpMessage]() {
            ...
            override protected def channelRead0(ctx: ChannelHandlerContext, msg: HttpMessage): Unit = { // If this handler is hit then no upgrade has been attempted and the client is just talking HTTP.
              ...
              pipeline.addAfter(thisCtx.name(),OBJECT_AGGREGATOR,new JHttpObjectAggregator(settings.maxRequestSize))

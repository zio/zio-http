 ...
 * Http.
 */
sealed trait Middleware[+RIn, -EIn, +AIn, -BIn, -ROut, +EOut, -AOut, +BOut] { self =>

  /**
   * Creates a new middleware that passes the output Http of the current
   * middleware as the input to the provided middleware.
   */
  final def >>>[RIn1 <: ROut, EIn1 >: EOut, AIn1 <: AOut, BIn1 >: BOut, ROut1, EOut1, AOut1, BOut1](
    other: Middleware[RIn1, EIn1, AIn1, BIn1, ROut1, EOut1, AOut1, BOut1],
  ): Middleware[RIn, EIn, AIn, BIn, ROut1, EOut1, AOut1, BOut1] = self andThen other

  /**
   * Applies self but if it fails, applies other.
   */
  final def <>[RIn1 <: RIn, EIn1, AIn0 >: AIn, BIn0 <: BIn, ROut1, EOut1, AOut0 <: AOut, BOut0 >: BOut](
    other: Middleware[RIn1, EIn1, AIn0, BIn0, ROut1, EOut1, AOut0, BOut0],
  ): Middleware[RIn1, EIn1, AIn0, BIn0, ROut1, EOut1, AOut0, BOut0] = self orElse other

  /**
   * Combines two middleware that don't modify the input and output types.
   */
  final def ++[R0 <: ROut, E0 >: EOut, A0 >: AIn <: AOut, B0 >: BOut <: BIn](
    other: Middleware[R0, E0, A0, B0, R0, E0, A0, B0],
  ): Middleware[RIn, EIn, A0, B0, R0, E0, A0, B0] =
    self combine other

  /**
   * Composes one middleware with another.
   */
  final def andThen[RIn1 <: ROut, EIn1 >: EOut, AIn1 <: AOut, BIn1 >: BOut, ROut1, EOut1, AOut1, BOut1](
    other: Middleware[RIn1, EIn1, AIn1, BIn1, ROut1, EOut1, AOut1, BOut1],
  ): Middleware[RIn, EIn, AIn, BIn, ROut1, EOut1, AOut1, BOut1] =
    new Middleware[RIn, EIn, AIn, BIn, ROut1, EOut1, AOut1, BOut1] {
      override def apply(http: Http[RIn, EIn, AIn, BIn]): Http[ROut1, EOut1, AOut1, BOut1] =
        other(self(http))
    }
   ...
   */
  def apply(http: Http[RIn, EIn, AIn, BIn]): Http[ROut, EOut, AOut, BOut]
  ...
  final def as[BOut0](
    bout: BOut0,
  ): Middleware[RIn, EIn, AIn, BIn, ROut, EOut, AOut, BOut0] =
    self.map(_ => bout)
   ...
   * into one.
   */
  final def combine[R0 <: ROut, E0 >: EOut, A0 >: AIn <: AOut, B0 >: BOut <: BIn](
    other: Middleware[R0, E0, A0, B0, R0, E0, A0, B0],
  ): Middleware[RIn, EIn, A0, B0, R0, E0, A0, B0] =
    self andThen other
   ...
   */
  final def contramap[AOut0](f: AOut0 => AOut): Middleware[RIn, EIn, AIn, BIn, ROut, EOut, AOut0, BOut] =
    self.contramapZIO[AOut0](a => UIO(f(a)))

  /**
   * Preprocesses the incoming value using a ZIO, for the outgoing Http.
   */
  final def contramapZIO[AOut0]: Middleware.PartialContraMapZIO[RIn, EIn, AIn, BIn, AOut, BOut, AOut0] =
    new Middleware.PartialContraMapZIO(self)

  /**
   * Delays the production of Http output for the specified duration
   */
  final def delay(duration: Duration): Middleware[RIn with Clock, EIn, AIn, BIn, ROut, EOut, AOut, BOut] =
    self.mapZIO(b => UIO(b).delay(duration))

  /**
   * Creates a new Middleware from another flipped the variance of RIn1 and EIn1
   * need to undo/confirm that
   */
  final def flatMap[
    RIn0 <: RIn,
    EIn0 >: EIn,
    ...
    f: BOut => Middleware[RIn0, EIn0, AIn0, BIn0, ROut0, EOut0, AOut0, BOut0],
  ): Middleware[RIn0, EIn0, AIn0, BIn0, ROut0, EOut0, AOut0, BOut0] =
    new Middleware[RIn0, EIn0, AIn0, BIn0, ROut0, EOut0, AOut0, BOut0] {
      override def apply(http: Http[RIn0, EIn0, AIn0, BIn0]): Http[ROut0, EOut0, AOut0, BOut0] =
  ...
  final def flatten[
    ...
    ev: BOut <:< Middleware[RIn1, EIn1, AIn0, BIn0, ROut1, EOut1, AOut0, BOut0],
  ): Middleware[RIn1, EIn1, AIn0, BIn0, ROut1, EOut1, AOut0, BOut0] =
    flatMap(identity(_))

  /**
   * Transforms the output type of the current middleware.
   */
  final def map[BOut0](f: BOut => BOut0): Middleware[RIn, EIn, AIn, BIn, ROut, EOut, AOut, BOut0] =
    self.flatMap(b => Middleware.succeed(f(b)))

  /**
   * Transforms the output type of the current middleware using effect function.
   */
  final def mapZIO[R1 <: RIn, E1 >: EIn, BOut0](
    f: BOut => ZIO[R1, E1, BOut0],
  ): Middleware[R1, E1, AIn, BIn, AOut, BOut0] =
    self.flatMap(b => Middleware.fromHttp(Http.fromZIO(f(b))))

  /**
   * Transforms the error type of the current middleware
   */
  // final def mapError[E1](f: E => E1): Middleware[R, E, AIn, BIn, AOut, BOut] =
  //   self.

  /**
   * Applies self but if it fails, applies other.
   */
  final def orElse[R1 <: RIn, E1, AIn0 >: AIn, BIn0 <: BIn, AOut0 <: AOut, BOut0 >: BOut](
   ...
   * completes
   */
  final def race[R1 <: RIn, E1 >: EIn, AIn1 >: AIn, BIn1 <: BIn, AOut1 <: AOut, BOut1 >: BOut](
  ...
  final def runAfter[R1 <: RIn, E1 >: EIn](effect: ZIO[R1, E1, Any]): Middleware[R1, E1, AIn, BIn, AOut, BOut] =
    self.mapZIO(bOut => effect.as(bOut))

  final def runBefore[R1 <: RIn, E1 >: EIn](effect: ZIO[R1, E1, Any]): Middleware[R1, E1, AIn, BIn, AOut, BOut] =
    self.contramapZIO(b => effect.as(b))

  /**
   * Applies Middleware based only if the condition function evaluates to true
   */
  final def when[AOut0 <: AOut](cond: AOut0 => Boolean): Middleware[RIn, EIn, AIn, BIn, AOut0, BOut] =
   ...
   * to true
   */
  final def whenZIO[R1 <: RIn, E1 >: EIn, AOut0 <: AOut](
...
object Middleware extends Web {
   ...
   */
  def fromHttp[R, E, A, B](http: Http[R, E, A, B]): Middleware[R, E, Nothing, Any, R, E, A, B] =
    new Middleware[R, E, Nothing, Any, R, E, A, B] {
   ...
   */
  def identity: Middleware[Any, Nothing, Nothing, Any, Nothing, Any, Any, Nothing] =
    new Middleware[Any, Nothing, Nothing, Any, Nothing, Any, Any, Nothing] {
      override def apply(
        http: Http[Any, Nothing, Nothing, Any],
      ): Http[Any, Nothing, Any, Nothing] =
        http.asInstanceOf[Http[Any, Nothing, Any, Nothing]]
    }
   ...
   */
  def succeed[B](b: B): Middleware[Any, Nothing, Nothing, Any, Any, Nothing, Any, B] = fromHttp(Http.succeed(b))

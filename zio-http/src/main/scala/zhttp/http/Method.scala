...
import zhttp.experiment.Route.RouteToken
...
object Method {
  ...
  implicit class MethodToRoute(method: Method) {
    def /(name: String): Route[Unit]         = Route.fromMethod(method) / name
    def /[A](token: RouteToken[A]): Route[A] = Route.fromMethod(method) / token
  }
}

...
object Headers extends HeaderConstructors {
  ...
  def apply(name: CharSequence, value: CharSequence): Headers = new Headers() {
    override def toChunk: Chunk[(CharSequence, CharSequence)] = Chunk((name, value))
  }
  ...
  def apply(iter: Iterable[Header]): Headers =
    new Headers() {
      override def toChunk: Chunk[(CharSequence, CharSequence)] = Chunk.fromIterable(iter)
    }

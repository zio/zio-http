...
object RequestContent {
  ...
  case class BinaryStream[R, E](data: ZStream[R, E, Byte]) extends RequestContent[R, E]
  def fromStream[R, E](data: ZStream[R, E, Byte]): RequestContent[R, E] = BinaryStream(data)

  case class Binary(data: Chunk[Byte]) extends RequestContent[Any, Nothing]
  def fromChunk(data: Chunk[Byte]): RequestContent[Any, Nothing] = Binary(data)

  case class BinaryN(data: ByteBuf) extends RequestContent[Any, Nothing]
  def fromByteBuf(byteBuf: ByteBuf): RequestContent[Any, Nothing] = BinaryN(byteBuf)

  case object Empty extends RequestContent[Any, Nothing]
  def empty: RequestContent[Any, Nothing] = Empty

  case class Text(text: String, charset: Charset) extends RequestContent[Any, Nothing]

...
object Content                   {
  private[zhttp] final case class CompleteContent(bytes: Chunk[Byte])               extends Content[Any, Nothing, Complete]
  private[zhttp] final case class BufferedContent[R, E](bytes: ZStream[R, E, Byte]) extends Content[R, E, Buffered]
  private[zhttp] case object EmptyContent                                           extends Content[Any, Nothing, Opaque]

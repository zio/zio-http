...
sealed trait DynamicServer {
  def add(app: Http.Total[Any, Throwable, Request, Response]): UIO[Id]

  def get(id: Id): UIO[Option[Http.Total[Any, Throwable, Request, Response]]]
...
object DynamicServer {
  ...
  def app: HttpApp[DynamicServer, Throwable] =
    Http.fromFunctionZIO { (req: Request) =>
      req.headerValue(APP_ID) match {
        case Some(id) =>
          get(id).map(_.getOrElse(Http.notFound))
        case None     =>
          ZIO.succeed(Http.notFound)
      }
    }.flatten
  ...
  def deploy[R](app: Http.Total[R, Throwable, Request, Response]): ZIO[DynamicServer with R, Nothing, String] =
    for {
      env <- ZIO.environment[R]
      id  <- ZIO.environmentWithZIO[DynamicServer](_.get.add(app.provideEnvironment(env)))
    } yield id

  def get(id: Id): ZIO[DynamicServer, Nothing, Option[Http.Total[Any, Throwable, Request, Response]]] =
  ...
  def live: ZLayer[Any, Nothing, DynamicServer] =
    ZLayer {
      for {
        ref <- Ref.make(Map.empty[Id, Http.Total[Any, Throwable, Request, Response]])
  ...
  final class Live(ref: Ref[Map[Id, Http.Total[Any, Throwable, Request, Response]]], pr: Promise[Nothing, Server])
      extends DynamicServer {
    def add(app: Http.Total[Any, Throwable, Request, Response]): UIO[Id] = for {
      id <- ZIO.succeed(UUID.randomUUID().toString)
      _  <- ref.update(map => map + (id -> app))
    } yield id

    def get(id: Id): UIO[Option[Http.Total[Any, Throwable, Request, Response]]] = ref.get.map(_.get(id))

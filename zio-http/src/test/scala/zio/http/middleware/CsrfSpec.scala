...
object CsrfSpec extends ZIOSpecDefault with HttpAppTestExtensions {
  ...
  override def spec         = suite("CSRF Middlewares")(
    test("x-token not present") {
      assertZIO(app(Request.make(headers = setCookie)))(equalTo(Status.Forbidden))
    },
    test("x-token mismatch") {
      assertZIO(app(Request.make(headers = setCookie ++ invalidXToken)))(
        equalTo(Status.Forbidden),
      )
    },
    test("x-token match") {
      assertZIO(app(Request.make(headers = setCookie ++ validXToken)))(
        equalTo(Status.Ok),
      )
    },
    test("app execution skipped") {
      for {
        ...
        _   <- app(Request.make(headers = setCookie ++ invalidXToken))
        res <- r.get

...
object TestClient {
  ...
  class Test(
    ...
    override protected[http] def requestInternal(
      body: Body,
      headers: Headers,
      ...
      queries: QueryParams,
      sslOption: Option[ClientSSLHandler.ClientSSLOptions],
      version: Version,
    )(implicit trace: Trace): ZIO[Any, Throwable, Response] = {
      for {
        response <- live
          .requestInternal(body, headers, hostOption, method, pathPrefix, portOption, queries, sslOption, version)
          .either
        // TODO Clean this up
        rez      <- response match {
          case Left(value)  =>
            responsesR.update(interactions =>
              Interaction(
                Request(
                  version = version,
                  method = method,
                  url = URL(pathPrefix, Location.Absolute(Scheme.HTTP, "localhost", port = portOption.getOrElse(-1))),
                  headers = headers,
                  body = body,
                ),
                Response(),
    ...
    override protected[http] def socketInternal[Env1](
      app: SocketApp[Env1],
      headers: Headers,
      hostOption: Option[String],
      pathPrefix: Path,

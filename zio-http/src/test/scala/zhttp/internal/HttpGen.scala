...
object HttpGen {
  def clientRequest[R](
    ...
    for {
      ...
      data    <- dataGen
    } yield ClientRequest(method = method, url = url, getHeaders = headers, data = data)
  ...
  def clientParamsForFileHttpData() = {
    ...
    } yield ClientRequest(method = method, url = url, getHeaders = headers, data = HttpData.fromFile(file))
  }

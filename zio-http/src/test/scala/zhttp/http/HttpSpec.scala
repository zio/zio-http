...
object HttpSpec extends DefaultRunnableSpec {
  def spec = suite("Http")(
    ...
    suite("fail")(
      test("should fail") {
        val a      = Http.fail(100)
        val actual = a.asResult(()).evaluateOrElse(-1)
    ...
    suite("evalAsEffect")(
      testM("should resolve") {
        val a      = Http.collect[Int] { case 1 => "A" }
        val actual = a.asResult(1).asEffect
        assertM(actual)(equalTo("A"))
      },
      testM("should complete") {
        val a      = Http.collect[Int] { case 1 => "A" }
        val actual = a.asResult(2).evaluateOrElse(-1).asEffect(-1).either

  ...
  def spec = suite("Http")(
    suite("flatMap")(
      test("should flatten") {
        ...
        val actual = app.execute(0).asOut
    ...
    suite("combine")(
      test("should resolve first") {
        ...
        val actual = (a +++ b).execute(1).asOut
        assert(actual)(isSuccess(equalTo("A")))
      },
      test("should resolve second") {
        val a      = Http.empty
        val b      = Http.succeed("A")
        val actual = (a +++ b).execute(()).asOut
        assert(actual)(isSuccess(equalTo("A")))
      },
      test("should resolve second") {
        val a      = Http.collect[Int] { case 1 => "A" }
        val b      = Http.collect[Int] { case 2 => "B" }
        val actual = (a +++ b).execute(2).asOut
        assert(actual)(isSuccess(equalTo("B")))
      },
      test("should not resolve") {
        val a      = Http.collect[Int] { case 1 => "A" }
        val b      = Http.collect[Int] { case 2 => "B" }
        val actual = (a +++ b).execute(3).asOut
        assert(actual)(isEmpty)
      },
      test("should be stack-safe") {
        val i      = 100000
    ...
    suite("asEffect")(
      testM("should resolve") {
        val a      = Http.collect[Int] { case 1 => "A" }
        val actual = a.execute(1).asOut.asEffect
        assertM(actual)(equalTo("A"))
      },
      testM("should complete") {
        val a      = Http.collect[Int] { case 1 => "A" }
        val actual = a.execute(2).asOut.asEffect.either

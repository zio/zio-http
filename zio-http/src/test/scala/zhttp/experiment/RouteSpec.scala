...
object RouteSpec extends DefaultRunnableSpec {
  def spec = suite("Route") {
    ...
    test("Route[Int]") {
      val route = Route.fromMethod(Method.GET) / Route[Int]
      assert(route.parse(!! / "1"))(isSome(equalTo(1))) && assert(route.parse(!! / "a"))(isNone)
    } +
      test("Route[String]") {
        val route = Route.fromMethod(Method.GET) / Route[String]
        assert(route.parse(!! / "a"))(isSome(equalTo("a")))
      } +
      test("Route[Boolean]") {
        val route = Route.fromMethod(Method.GET) / Route[Boolean]
      ...
      test("Route[Int] / Route[Int]") {
        val route = Route.fromMethod(Method.GET) / Route[Int] / Route[Int]
      ...
      test("Route[Int] / c") {
        val route = Route.fromMethod(Method.GET) / Route[Int] / "c"

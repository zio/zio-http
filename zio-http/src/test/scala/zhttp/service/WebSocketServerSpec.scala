...
object WebSocketServerSpec extends HttpRunnableSpec {
  ...
  override def spec = suiteM("Server") { app.as(List(websocketSpec)).useNow }
    .provideCustomLayerShared(env) @@ timeout(30 seconds)

  val websocketSpec = suite("WebsocketSpec")(
    suite("WebSocketFrameSpec") {
      testM("binary") {

...
object TransportSpec extends HttpRunnableSpec(9091) {
  ...
  val appWithNio  = serve(http, 9092, Transport.Nio)

  val appWithAutoLayer = appWithAuto.toLayer
  val appWithNioLayer  = appWithNio.toLayer

  override def spec = suite("Server")(
    suite("Transport With Nio")(
    ...
    suite("Transport With Nio")(
      testM("200 response with Nio transport mode") {
        val actual = status(!! / "success")
        assertM(actual)(equalTo(Status.OK))
      },
    ).provideCustomLayerShared(env ++ appWithNioLayer),

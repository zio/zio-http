...
abstract class HttpRunnableSpec(port: Int) extends DefaultRunnableSpec {
  ...
  def requestPath(path: Path): ZIO[EventLoopGroup with ChannelFactory, Throwable, UHttpResponse] =
    Client.request(Method.GET -> URL(path, Location.Absolute(Scheme.HTTP, "localhost", port)),SslClientOptions.DefaultSSLClient)
  ...
  def request(
    ...
    Client.request(Request(method -> URL(path, Location.Absolute(Scheme.HTTP, "localhost", port)), headers, data),SslClientOptions.DefaultSSLClient)
  }
}
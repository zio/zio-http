...
object ServerSpec extends HttpRunnableSpec {
  ...
  def dynamicAppSpec = suite("DynamicAppSpec")(
    suite("success")(
      test("status is 200") {
        val status = testClient.flatMap(client => Http.ok.deploy(client).status.run())
        assertZIO(status)(equalTo(Status.Ok))
      },
      test("status is 200") {
        val res = testClient.flatMap(client => Http.text("ABC").deploy(client).status.run())
        assertZIO(res)(equalTo(Status.Ok))
      },
      test("content is set") {
        val res = testClient.flatMap(client => Http.text("ABC").deploy(client).body.mapZIO(_.asString).run())
    ...
    suite("not found") {
      ...
      test("status is 404") {
        ...
        test("header is set") {
          val res = testClient.flatMap(client => app.deploy(client).headerValue(HeaderNames.contentLength).run())
      ...
      suite("error") {
        ...
        test("status is 500") {
          ...
          test("content is set") {
            val res = testClient.flatMap(client => app.deploy(client).body.mapZIO(_.asString).run())
        ...
        test("status is 200") {
          ...
          test("body is ok") {
            val res = testClient.flatMap(client =>
              app.deploy(client).body.mapZIO(_.asString).run(body = Body.fromString("ABC")),
            )
            assertZIO(res)(equalTo("ABC"))
          } +
          test("empty string") {
            val res =
              testClient.flatMap(client => app.deploy(client).body.mapZIO(_.asString).run(body = Body.fromString("")))
            assertZIO(res)(equalTo(""))
          } +
          test("one char") {
            val res =
              testClient.flatMap(client => app.deploy(client).body.mapZIO(_.asString).run(body = Body.fromString("1")))
          ...
          test("data") {
            ...
            val res        = testClient.flatMap(client =>
              app.deploy(client).body.mapZIO(_.asChunk.map(_.length)).run(body = Body.fromStream(dataStream)),
            )
      ...
      suite("headers") {
        ...
        test("headers are set") {
          val res = testClient.flatMap(client => app.deploy(client).headerValue("Foo").run())
      ...
      suite("decompression") {
        val app     = Http
          .fromZIO(testClient)
          .flatMap(client =>
            Http.collectZIO[Request] { case req => req.body.asString.map(body => Response.text(body)) }.deploy(client),
          )
  ...
  def requestSpec = suite("RequestSpec") {
    ...
    test("has content-length") {
      check(Gen.alphaNumericString) { string =>
        val res =
          testClient.flatMap(client => app.deploy(client).body.mapZIO(_.asString).run(body = Body.fromString(string)))
      ...
      test("POST Request.getBody") {
        ...
        val res = testClient.flatMap(client =>
          app.deploy(client).status.run(path = !!, method = Method.POST, body = Body.fromString("some text")),
        )
        assertZIO(res)(equalTo(Status.Ok))
      } +
      test("body can be read multiple times") {
        ...
        val res = testClient.flatMap(client =>
          app.deploy(client).status.run(method = Method.POST, body = Body.fromString("some text")),
        )
        assertZIO(res)(equalTo(Status.Ok))
      }
  }
  ...
  def responseSpec = suite("ResponseSpec")(
    test("data") {
      check(nonEmptyContent) { case (string, data) =>
        val res = testClient.flatMap(client => Http.fromBody(data).deploy(client).body.mapZIO(_.asString).run())
        assertZIO(res)(equalTo(string))
      }
    },
    test("data from file") {
      val res =
        testClient.flatMap(client => Http.fromResource("TestFile.txt").deploy(client).body.mapZIO(_.asString).run())
    ...
    test("status") {
      checkAll(HttpGen.status) { case status =>
        val res = testClient.flatMap(client => Http.status(status).deploy(client).status.run())
    ...
    test("header") {
      check(HttpGen.header) { case header @ (name, value) =>
        val res = testClient.flatMap(client => Http.ok.addHeader(header).deploy(client).headerValue(name).run())
        assertZIO(res)(isSome(equalTo(value)))
      }
    },
    test("text streaming") {
      val res = testClient.flatMap(client =>
        Http.fromStream(ZStream("a", "b", "c")).deploy(client).body.mapZIO(_.asString).run(),
      )
      assertZIO(res)(equalTo("abc"))
    },
    test("echo streaming") {
    ...
    test("file-streaming") {
      ...
      val res  = testClient.flatMap(client =>
        Http.fromStream(ZStream.fromPath(Paths.get(path))).deploy(client).body.mapZIO(_.asString).run(),
      )
      assertZIO(res)(equalTo("foo\nbar"))
    },
    suite("html")(
      test("body") {
        val res = testClient.flatMap(client =>
          Http.html(html(body(div(id := "foo", "bar")))).deploy(client).body.mapZIO(_.asString).run(),
        )
        assertZIO(res)(equalTo("""<!DOCTYPE html><html><body><div id="foo">bar</div></body></html>"""))
      },
      test("content-type") {
        val app = Http.html(html(body(div(id := "foo", "bar"))))
        val res = testClient.flatMap(client => app.deploy(client).headerValue(HeaderNames.contentType).run())
    ...
    suite("content-length")(
      suite("string") {
        test("unicode text") {
          val res = testClient.flatMap(client => Http.text("äöü").deploy(client).contentLength.run())
          assertZIO(res)(isSome(equalTo(6L)))
        } +
          test("already set") {
            val res = testClient.flatMap(client =>
              Http.text("1234567890").withContentLength(4L).deploy(client).contentLength.run(),
            )
    ...
    suite("memoize")(
      test("concurrent") {
        ...
        for {
          ...
          client   <- testClient
          actual   <- ZIO.foreachPar(0 to size)(_ => Http.response(response).deploy(client).status.run())
      ...
      test("update after cache") {
        ...
        for {
          ...
          client <- testClient
          actual <- Http.response(res).withServer(server).deploy(client).headerValue(HeaderNames.server).run()
  ...
  def requestBodySpec = suite("RequestBodySpec")(
    test("POST Request stream") {
      ...
      check(Gen.alphaNumericString) { c =>
        assertZIO(
          testClient.flatMap(client =>
            app.deploy(client).body.mapZIO(_.asString).run(path = !!, method = Method.POST, body = Body.fromString(c)),
          ),
        )(
          equalTo(c),
        )
  ...
  def serverErrorSpec = suite("ServerErrorSpec") {
    ...
    test("status is 500") {
      ...
      test("header is set") {
        val res = testClient.flatMap(client => app.deploy(client).headers.run().map(_.headerValue("Content-Length")))
        assertZIO(res)(isSome(anything))
      }
  }

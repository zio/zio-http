...
object HttpsHelloWorld extends App {
  ...
  /**
   * sslcontext can be created using SslContexBuilder. In this example an inbuilt API using keystore is used For testing
   * this example using curl, setup the certificate named "localhost.cert" from resources for the OS Alternatively you
   * can create the keystore and certificate using the following link

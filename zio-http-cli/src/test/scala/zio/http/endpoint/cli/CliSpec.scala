...
object CliSpec extends ZIOSpecDefault {
        ...
        ZLayer.scoped {
            (for {
                ...
                _ <- testClient.addHandler {
                    ...
                    case Request(body, headers, Method.GET, _, _, _) if headers.headOption.map(_.renderedValue) == Some("fromURL") =>
                        ZIO.succeed(Response(Status.Ok, headers, body))
                    case Request(body, _, Method.GET, _, _, _) =>
                        for {
                            text <- body
                                .asMultipartForm.map(_.formData)
                                .map(_.map(_.stringValue.toString())).map(_.toString())
    ...
    val cliApp: CliApp[Any, Throwable, CliRequest] =
        HttpCliApp
            .fromEndpoints("simple", "1.0.0", HelpDoc.Span.empty, endpoints, "localhost", 0, client = CliZLayerClient(testClient)).cliApp

    override def spec =
        suite("HttpCLI")(
            suite("GenTests")(
                test("fromEndpoint generates the correct CliEndpoint") {
                    check(EndpointGen.anyCliEndpoint) {
                    case CliRepr(endpoint, repr) => assertTrue(endpoint == repr)
                    }
                },
                test("CliEndpoint generates correct Options") {
                    check(OptionsGen.anyCliEndpoint) {
                        case CliRepr(options, repr) => {
                            assertTrue(HttpCommand.addOptionsTo(repr).helpDoc.toPlaintext() == options.helpDoc.toPlaintext()

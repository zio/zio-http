...
object CommandGen {
    ...
    def getCommand(cliEndpoint: CliEndpoint): HelpDoc = {      
        ...
        val urlOptions = cliEndpoint.url
            .filter {
        ...
        val headersOptions = cliEndpoint.headers
            .filter {
                case _: HttpOptions.Constant => false
                case _ => true
            }
        ...
        val bodyOptions = cliEndpoint.body.map {
            case HttpOptions.Body(name, _, schema, _) =>
    ...
    def getName(name: String, prefix: String): String =
        ...
        else if (name == "") s"-$prefix" else s"--$prefix-$name"
    ...
    def canBeTested(schema: Schema[_]): Boolean =
        schema match {
            case record: Schema.Record[_] => !record.fields
                .map(_.schema) 
                .map(getPrimitive(_) != "non primitive")
                .contains("non primitive")
            case enumeration: Schema.Enum[_] => !enumeration.cases
                .map(_.schema) 
                .map(getPrimitive(_))
                .contains("non primitive")
            case _ => true
    ...
    def bodyParameters(name: String, schema: Schema[_]): String =
        schema match {
            ...
            case record: Schema.Record[_] if canBeTested(record) => {
                record.fields.map {
                    case field => s"${bodyParameters(s"$name.${field.name}", field.schema)}"
                }.mkString(" ")
            }
            case enumeration: Schema.Enum[_] if canBeTested(enumeration) =>
                enumeration.cases.map {
                    case enumCase => s"${bodyParameters(s"$name.${enumCase.id}", enumCase.schema)}"
                }.mkString("|")

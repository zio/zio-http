...
object EndpointGen {
    ...
    def fromInputCodec[Input](doc: Doc, input: HttpCodec[CodecType, Input]): Endpoint[Input, ZNothing, ZNothing, EndpointMiddleware.None] =
    ...
    lazy val anyContent: Gen[Any, CliReprOf[Codec[_]]] =
        anySchema
            .zip(Gen.option(Gen.alphaNumericStringBounded(1, 30))).zip(Gen.option(anyMediaType))
    ...
    lazy val anyHeader: Gen[Any, CliReprOf[Codec[_]]] =
        Gen.alphaNumericStringBounded(1, 30).zip(anyTextCodec).map { case (name, codec) => 
            CliRepr(HttpCodec.Header(name, codec), codec match {
    ...
    lazy val anyQuery: Gen[Any, CliReprOf[Codec[_]]] = 
        Gen.alphaNumericStringBounded(1, 30).zip(anyTextCodec).map { case (name, codec) => 
            CliRepr(HttpCodec.Query(name, codec), codec match {

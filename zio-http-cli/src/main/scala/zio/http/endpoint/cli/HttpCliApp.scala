      ...
      CliApp.make(
        ...
        for {
          ...
          response <- client match {
            case CliZIOClient(client)    => client.simple(request)
            case CliZLayerClient(client) => Client.simple(request).provide(client)
            case DefaultClient()         => Client.simple(request).provide(Client.default)
          }

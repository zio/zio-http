 ...
 *
 * Fixtures are plain JVM `try`/`finally` brackets over the public server path
 * (`withLoomServer` / `withJdkServer` take `Int => TestResult`): only ZIO Test
 * remains as the test framework, with no ZIO effects or DI in this spec.
 */
object ClientResponseCapSpec extends ZIOSpecDefault {
  ...
  def spec = suite("ClientResponseCapSpec")(
  ...
  /**
   * Plain bracket over the public server path: construction and start happen as
   * direct calls, and `shutdownAndWait()` always runs — even when the client or
   * the assertions throw.
   */
  private def withLoomServer(use: Int => TestResult): TestResult = {
    val handle = LoomServer(Connector(bind = BindAddress.localhost(0))).serve(BigRoutes, Context.empty)
    try {
  ...
  /**
   * Plain bracket over the JDK-built-in HTTP/1.1 server: `stop(0)` always runs,
   * even when the client or the assertions throw.
   */
  private def withJdkServer(use: Int => TestResult): TestResult = {
    ...
    server.start()
    try use(server.getAddress.getPort)

 ...
 *
 * Each authority (scheme/host/port) gets at most `pool.maxPerHost`
 * connections (and `pool.maxTotal` globally); every connection carries
 * sequential exchanges with lazily-streamed response bodies. Pool mechanics,
 * honored exactly per the [[PoolConfig]] idle-reclamation contract:
 ...
 *   - queued borrowers wait at most `effectiveConnectTimeout`, then fail with
 *     a [[TimeoutException]] naming the acquire (bounded wait, never a hang);
 ...
 *   - in-flight cancel sends RST_STREAM (`H2Error.Code.CANCEL`) then closes
 *     the socket; the connection is evicted, never repooled;
 *   - server GOAWAY / server-side close / timeouts poison the connection: it
 *     is evicted on first observation and never handed out again (no
 *     black-hole reuse). Failures before the request HEADERS flush retry once
 *     on a fresh connection (the server never saw the request); anything
 *     later propagates - no retry policy beyond pool mechanics.
 *
 * Asymmetry (deliberate): request upload stays fully buffered
 * (`Body.toArray`, T14 semantics); response download streams frame-by-frame
 * through a lazily-pulled [[Body]] (one 16 KiB carry buffer live at a time).
 * The pool slot stays checked out until the lazy body is fully consumed (or
 * the exchange is cancelled / fails): abandoning a body pins its connection
 * until the socket deadline fires, so always consume or cancel.
 *
 ...
 * based, and [[CancellableExchange]] runs on the caller's thread. `close()`
 * is idempotent; it releases idle connections, fails future checkouts with
...
final class PooledLoomH2Client private (
  ...
  /**
   * Starts an exchange, acquiring its pool slot immediately. [[CancellableExchange.await]]
   * blocks the calling (virtual) thread for the response HEADERS; the body -
   * unless empty - streams lazily and holds the slot until fully consumed.
   */
  ...
  private def route(request: Request): (PoolKey, String, Int, String) = {
    ...
    val scheme = url.scheme.map(_.text).getOrElse(
  ...
  /** Pops a live idle connection for `key`; closes dead ones. Caller holds `lock`. */
  ...
  final class CancellableExchange private[PooledLoomH2Client] (
    ...
    private def awaitOnce(): Response = {
      ...
      val url         = request.url
      val schemeText  = url.scheme.map(_.text).getOrElse(
  ...
  /**
   * H2C/TLS-h2 both failed under the fallback policy: the slot pair is
   * already released; delegate one-shot to the JDK h1.1 leg. An internal
   * control signal, never surfaced (callers translate it before it escapes).
   */
  private case class H11FallbackSignal(host: String, port: Int, cause: Throwable) extends IOException(
...
/** Pool authority key (scheme/host/port). Top-level so the pool class can use it unqualified. */

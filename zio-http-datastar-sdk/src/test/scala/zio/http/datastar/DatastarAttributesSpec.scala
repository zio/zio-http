...
object DatastarAttributesSpec extends ZIOSpecDefault {
  ...
  override def spec = suite("DatastarAttributesSpec")(
    test("basic attribute rendering") {
      ...
      val expected   = "data-show=\"$mySignal\""
      assertTrue(rendered.contains("data-text=\"Hello\""), rendered.contains(expected))
    },
    test("data bind") {
    ...
    test("nested signal via builder API") {
      ...
      val expected = "data-show=\"$bar.foo\""
      assertTrue(view.render.contains(expected))
    },
    ...
    test("data-ignore-self boolean attribute short form") {
      ...
      val rendered = view.render
      assertTrue(rendered.contains("data-ignore__self"))
    },
    test("data-json-signals terse modifier") {
    ...
    test("data-on-signal-patch with debounce and throttle modifiers") {
      ...
      assertTrue(
        rendered.contains(
          "data-on-signal-patch__debounce.200ms.leading__throttle.300ms.noleading.trailing=\"handle()\"",
        ),
      )
    },

...
final case class EndpointGen(config: Config) {
            ...
            if (schema.isPrimitive) {
              if (config.generateSafeTypeAliases)
                ...
                schemaToField(schema, openAPI, name, Chunk.empty).foreach {
                  case Code.Field(_, fieldType: Code.Primitive) =>
  ...
  def schemaToCode(
    ...
    schema match {
      ...
      case JsonSchema.Integer(_)     => aliasedSchemaToCode(openAPI, name, schema)
      case JsonSchema.String(_, _)   =>
        aliasedSchemaToCode(
          openAPI,
          name,
          schema,
        ) // this could maybe be im proved to generate a string type with validation
      ...
      case JsonSchema.Number(_)               => aliasedSchemaToCode(openAPI, name, schema)

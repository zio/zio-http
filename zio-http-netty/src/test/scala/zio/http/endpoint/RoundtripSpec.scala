...
import zio.http.forms.ImageMetadata
...
object RoundtripSpec extends ZIOHttpSpec {
  val testLayer: ZLayer[Any, Throwable, Server & Client & Scope] =
    ZLayer.make[Server & Client & Scope](
      ...
      NettyClient.customized.map(env => ZEnvironment(env.get)),
      ClientDriver.shared,
  ...
  def spec: Spec[Any, Any] =
    suiteAll("RoundtripSpec") {
      ...
      NettyServer.customized,
      ZLayer.succeed(Server.Config.default.onAnyOpenPort.enableRequestStreaming),
      NettyClient.customized.map(env => ZEnvironment(env.get @@ clientDebugAspect)) >>>

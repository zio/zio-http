...
class ClientBenchmark {
  ...
  def tearDown(): Unit = {
    val stopServer = (for {
      client <- ZIO.service[Client]
      _      <- client.quick(Request(url = url"http://localhost:8080/shutdown"))
    } yield ()).provide(ZClient.default)
    run(stopServer)
    rtm.shutdown0()
  }
  ...
  def zhttpChunkBenchmark(): Any = run {
    ...
    ZIO.serviceWithZIO[Client] { client =>
      client.quickWithZIO(req)(_.body.asChunk).repeatN(100)
    }
  }
  ...
  def zhttpStreamToChunkBenchmark(): Any = run {
    val req = if (path == "small") smallRequest else largeRequest
    ZIO.serviceWithZIO[Client] { client =>
      client.quickWithZIO(req)(_.body.asStream.runCollect).repeatN(100)
    }
  }
}

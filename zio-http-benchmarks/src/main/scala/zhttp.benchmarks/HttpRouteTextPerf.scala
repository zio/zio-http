...
class HttpRouteTextPerf {
  ...
  private val app: HttpApp.HttpApp[Any, Nothing] = HttpApp.text("HELLO WORLD")
  private val req: Request                       = Request(Method.GET -> URL(Root))
  private val httpProgram                        = ZIO.foreach_(0 to 1000) { _ => app.asHttp.execute(req).evaluate.asEffect }

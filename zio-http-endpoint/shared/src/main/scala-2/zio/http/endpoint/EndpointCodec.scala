...
import zio.http.{Body, ContentType, Headers, Request, Response, Status}
...
private[endpoint] object EndpointCodec {
  ...
  def encodeResponse[A](codec: HttpCodec[CodecKind.Response, A], value: A): Response =
    encodeResponse(codec, value, 200)
    ...
    encodeResponseValue(codec, value) match {
      case Right(parts)  =>
        val base = Response(
          ...
          body = parts.body.getOrElse(Body.empty),
          version = zio.http.Version.`HTTP/1.1`,
        )
        parts.headers.foldLeft(base) { case (response, (name, rendered)) => response.addHeader(name, rendered) }

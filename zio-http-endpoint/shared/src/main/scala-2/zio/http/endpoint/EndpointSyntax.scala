 ...
 *   - `.call(client, pathInput, input)` renders the full request (method,
 *     path, query, headers, body) via `EndpointBridge.buildRequest`; the
 *     two-argument `.call(client, input)` shorthand is available only for
 *     root-path endpoints (`PathInput =:= Unit`, see
 *     [[UnitPathEndpointSyntax]]).
...
/**
 * Syntax for root-path endpoints (`PathInput =:= Unit`): adds the
 * two-argument `.call(client, input)` shorthand, which renders the request
 * with an empty path input. Brought into scope by the same plain
 * `import zio.http.endpoint._` as [[EndpointSyntax]].
 */
class UnitPathEndpointSyntax[Input, Err, Output, Auth <: AuthType](
   ...
   * decoded `Err | Output` union. Shorthand for
   * `call(client, (), input)`.
   *
   * Requires Eithers TC instance for combining error and output responses.
   */

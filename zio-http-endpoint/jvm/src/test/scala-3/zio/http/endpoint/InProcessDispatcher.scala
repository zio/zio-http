 ...
 * codec: `route.handler` is exactly the [[zio.http.Handler]] that
 * `.implement` produced (built on [[EndpointCodec]]'s real JSON encode/decode
 * over [[zio.http.Body]]), so dispatching through it exercises the genuine
 * HTTP encode/decode path, not a shortcut that calls the user handler
 * directly.
 */
private[endpoint] object InProcessDispatcher {

  /** Dispatches `request` through `route`, resolving a `Halt` to its wrapped [[Response]]. */
  def dispatch(route: Route[Any], request: Request): Response = {
    ...
    route.handler.handle(request, Context.empty, vars, Scope.global) match {
      ...
      case Halt(response)     => response
    }
  }

  /** Wraps `route` as a [[Client]] that dispatches in-process, for exercising `.call`. */

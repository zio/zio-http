...
object EndpointContextParamSpec extends ZIOSpecDefault {
  ...
  private val oneCtxRoute: zio.http.Route[Greeter] =
    endpoint.implement[EndpointResultHandler.Id] { (name: String, greeter: Greeter) =>
      s"${greeter.prefix} $name"
    }

  private val twoCtxRoute =
    endpoint.implement[EndpointResultHandler.Id] { (name: String, greeter: Greeter, auditor: Auditor) =>

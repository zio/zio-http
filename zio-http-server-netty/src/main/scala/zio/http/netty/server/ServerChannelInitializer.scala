...
import io.netty.handler.codec.compression.{CompressionOptions, DeflateOptions, StandardCompressionOptions}
  ...
  override def initChannel(channel: Channel): Unit = {
    ...
    cfg.responseCompression.foreach(ops => {
      pipeline.addLast(
        ...
        new HttpContentCompressor(ops.contentThreshold, ops.options.map(ServerChannelInitializer.compressionOptionsToNetty): _*),
      )
    })
...
object ServerChannelInitializer {
  ...
  def compressionOptionsToNetty(compressionOptions: Server.Config.CompressionOptions): DeflateOptions =
    compressionOptions.kind match {
      case Server.Config.CompressionOptions.CompressionType.GZip    =>
        StandardCompressionOptions.gzip(compressionOptions.level, compressionOptions.bits, compressionOptions.mem)
      case Server.Config.CompressionOptions.CompressionType.Deflate =>

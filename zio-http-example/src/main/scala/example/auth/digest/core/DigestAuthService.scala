...
object DigestAuthService {
  val live: ZLayer[HashService & NonceService, Nothing, DigestAuthService] =
    ZLayer.fromFunction(in =
      (hashService: HashService, nonceService: NonceService) =>
        new DigestAuthService {
          ...
          private def calculateResponse(
            ...
            val responseData = qop match {
              case Some(qop) =>
                s"$nonce:${nc.get}:${cnonce.get}:${qop.name}:$ha2"
              case None      =>
                s"$nonce:$ha2"
            }
          ...
          def validateCredentials(response: String)(
            ...
            for {
              ...
              isUsed <- nonceService.isNonceUsed(nonce, nc.get)
              _      <- ZIO.when(isUsed)(ZIO.fail(ReplayAttack(nonce, nc.get)))
              ...
              // Mark nonce as used
              _ <- nonceService.markNonceUsed(nonce, nc.get)

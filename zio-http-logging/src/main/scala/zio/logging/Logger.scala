...
final case class Logger(transports: List[LoggerTransport]) extends LoggerMacroExtensions { self =>
   ...
   */
  def detectLevelFromProps(env: String): Logger = {
    println("Log level: " + LogLevel.detectFromProps(env))
//    withLevel(LogLevel.detectFromProps(env).getOrElse(LogLevel.Error))
...
object Logger {
  private[zio] val detectedLevel: LogLevel = LogLevel.detectFromProps("ZIOHttpLogLevel").getOrElse(LogLevel.Error)

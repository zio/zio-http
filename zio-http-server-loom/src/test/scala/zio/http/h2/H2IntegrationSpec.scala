...
object H2IntegrationSpec extends ZIOSpecDefault {
  override def spec: Spec[TestEnvironment & Scope, Any] =
    suite("H2IntegrationSpec")(
      ...
      test("handler returning Halt uses its embedded response status") {
        val routes = Routes(
          Route(
            RoutePattern.GET,
            handler { (_: Request) =>
        ...
        withServer(routes) { port =>
          withClient(port) { client =>
            for {
              response <- ZIO.attemptBlocking(client.roundTrip("GET", "/", Chunk.empty, streamId = 1))
            } yield assertTrue(response.status == 201)
          }
        }
      },
      test("Protocol.H3 connector throws UnsupportedOperationException on start") {
        ZIO
          .attempt {
            val tlsCfg = TlsConfig(
              certChain  = TlsSource.PemString(Secret("dummy-cert")),
              privateKey = TlsSource.PemString(Secret("dummy-key")),
            )
            new H2Transport(
              Routes(Route(RoutePattern.GET, Handler.succeed(Response.ok))),
              Context.empty,
              Connector(bind = BindAddress.localhost(0), protocol = Protocol.H3(tlsCfg)),
              DefectHandler.default,
            ).start()
          }
          .either
          .map(r => assertTrue(r.left.exists(_.isInstanceOf[UnsupportedOperationException])))
      },
      test("BindAddress.Unix connector throws UnsupportedOperationException on start") {
        ZIO
          .attempt {
            new H2Transport(
              Routes(Route(RoutePattern.GET, Handler.succeed(Response.ok))),
              Context.empty,
              Connector(bind = BindAddress.Unix(java.nio.file.Paths.get("/tmp/zio-http-test.sock"))),
              DefectHandler.default,
            ).start()
          }
          .either
      ...
      test("LoomServer with multiple connectors serves on both ports") {
        val routes = Routes(Route(RoutePattern.GET, Handler.succeed(Response.ok)))
        ZIO
          .acquireRelease(
            ZIO.attempt(
              LoomServer(Connector(bind = BindAddress.localhost(0)))
                .addConnector(Connector(bind = BindAddress.localhost(0)))
                .serve(routes, Context.empty),
            ),
          )(h => ZIO.succeed(h.shutdownAndWait()))
          .flatMap { handle =>
            val ports = handle.bindings.map {
              _.address match {
            ...
            ZIO
              .collectAll(
                ports.map { port =>
                  withClient(port) { client =>
                    ZIO.attemptBlocking(client.roundTrip("GET", "/", Chunk.empty, streamId = 1))
                      .map(r => assertTrue(r.status == 200))
                  }
                },
              )

  ...
  override def spec: Spec[TestEnvironment & Scope, Any] =
    suite("H2ConnectionControlSpec")(
      ...
      test("lastPeerStreamId decreases after handleGoAway with lower stream id") {
        ZIO.attemptBlocking {
          val (control, _) = makeControl()
          control.handleGoAway(H2Frame.GoAway(lastStreamId = 10, errorCode = H2Error.Code.NO_ERROR, debugData = Chunk.empty))
      ...
      test("lastPeerStreamId does not increase after handleGoAway with higher stream id") {
        ZIO.attemptBlocking {
          val (control, _) = makeControl()
          control.handleGoAway(H2Frame.GoAway(lastStreamId = 5, errorCode = H2Error.Code.NO_ERROR, debugData = Chunk.empty))
          control.handleGoAway(H2Frame.GoAway(lastStreamId = 99, errorCode = H2Error.Code.NO_ERROR, debugData = Chunk.empty))

  ...
  override def spec: Spec[TestEnvironment & Scope, Any] =
    suite("FlowControllerSpec")(
      ...
      test("registerStream and removeStream control stream visibility") {
        ...
        ZIO.attempt(fc.streamWindow(1)).either.map { result =>
          assertTrue(
            registeredWindow == 65535,
            result.left.exists(_.isInstanceOf[NoSuchElementException]),
          )
        }
      },
      test("window overflow throws FlowControlException") {
        ...
        ZIO.attempt(fc.applyWindowUpdate(0, 6)).either.map { result =>
          assertTrue(result.left.exists(_.isInstanceOf[FlowController.FlowControlException]))
        }
      },

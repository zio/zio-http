        ...
        withRawServer(routes) { port =>
          ZIO.attemptBlocking {
            ...
            try {
              ...
              client.sendRaw(FrameCodec.encode(Data(streamId = 3, data = Chunk.fromArray("hi".getBytes), endStream = true)).toArray)
        ...
        withRawServer(routes) { port =>
          ZIO.attemptBlocking {
            ...
            try {
              ...
              // Server stream handler throws; connection should either close or send an error
              client.socket.setSoTimeout(3000)
              val errOrClosed =
                try {

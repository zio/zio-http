    ...
    suite("H2SmokeAndExtraSpec")(
      ...
      test("LoomServer.withDefectHandler returns server using new defect handler") {
        ...
        val customDefect = new DefectHandler {
          override def handleDefect(request: zio.http.Request, throwable: Throwable): Response | Halt =
            Response(Status.ServiceUnavailable): Response | Halt
        }
        val routes = Routes(
          Route(
            RoutePattern.GET,
        ...
        val server = zio.http.LoomServer()
      ...
      test("FlowController consumeSendWindow waits when window is zero then unblocks") {
        ZIO.attemptBlocking {
          ...
          val unblockThread = Thread.ofVirtual().start(() => {
            Thread.sleep(50)
